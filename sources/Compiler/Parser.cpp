...
const std::array<Token::Type, 6> Parser::declaration_tokens = {
	Token::Type::Abstract,
	Token::Type::Class,
	Token::Type::Const,
	Token::Type::Final,
...
Ptr<AST::ClassDeclaration> Parser::class_declaration()
{
	bool is_final = consume(Token::Type::Final);
	bool is_abstract = consume(Token::Type::Abstract);

	if (is_final && is_abstract) {
		Log::error("Class can not be final and abtract at the same time");
		return nullptr;
	}

	MUST_CONSUME(Token::Type::Class);

	auto name = identifier();
	if (!name) return nullptr;

	if (consume(Token::Type::Extends)) {
		auto parent = expression();
		if (!parent) return nullptr;

		if (!detail::is<AST::Identifier, AST::MemberExpression>(parent)) {
			Log::error("Unexpected {}, expected identifier or member expression", parent->class_name());
			return nullptr;
		}
	}
	...
	while (!done() && !peek(Token::Type::RightBrace)) {
		auto visibility = consume();
		if (visibility.type != Token::Type::Public || visibility.type != Token::Type::Protected || visibility.type != Token::Type::Private) {
			Log::error("Unexpected token {}, expected 'public', 'protected' or 'private'", visibility);
			return nullptr;
		}
		...
		switch (m_current_token.type) {
			...
			case Token::Type::Function:
				...
				break;
			default:
				Log::error("Unexpected token {}, expected 'const', 'let' or 'function'", m_current_token);
				return nullptr;
		}
	}
	...
	return makeNode<AST::ClassDeclaration>(
		is_final
	);
}
